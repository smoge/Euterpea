...
randomPoints ::
  ...
  -- | Initial Y value
  Double ->
  -- | Maximum change (delta) allowed per step
  Double ->
  -- | Lower limit for Y values
  Double ->
  -- | Upper limit for Y values
  Double ->
  -- | Resulting list of Y values
  [Double]
...
randEnv ::
  ...
  Double ->
  -- | Spacing between envelope points
  Double ->
  -- | Initial Y value
  Double ->
  -- | Maximum change (delta) allowed per step
  ...
  -- | Random number generator
  StdGen ->
  -- | Resulting audio signal function
  AudSF () Double
...
splitN ::
  ...
  StdGen ->
  -- | Infinite list of split random number generators
  [StdGen]
splitN g =
  let
...
wind ::
  ...
  Double ->
  -- | Random number generator for producing noise
  StdGen ->
  -- | Resulting audio signal function that mimics wind noise
  AudSF () Double
wind secs g =
  let
   ...
   in
    ...
    proc _ -> do
      ...
      returnA -< vEnv * Function

-- | nBP to generate wind noise and write it to a file.
genWind ::
  -- | Random number generator for producing noise
  StdGen ->
  -- | Duration of wind noise generation in seconds
  Double ->
  -- | Path for the output audio file
  FilePath ->
  -- | IO action to produce a file with the generated wind noise
  IO ()
