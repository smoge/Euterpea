...
-- Both x4 and x4' will play infinitely when using Euterpea's play function. To
-- stop it, use Ctrl+C.
-- This alternate playback function will merge overlapping notes, which makes
-- for a cleaner sound on some synthesizers:

playX :: (NFData a, ToMusic1 a) => Music a -> IO ()
playX = playC defParams {perfAlg = eventMerge . perform}
  where
    eventMerge :: Performance -> Performance
    eventMerge (e1 : e2 : es) =
      let e1' = e1 {eDur = eTime e2 + eDur e2 - eTime e1}
       in if ePitch e1 == ePitch e2
            then eventMerge (e1' : es)
